...
const initialState: AnimalStateType = {
  animals: [
        {    id: 1,
            name: "Frog",
            type: "mammals",
            img: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTL-eRbYyu5BKUZNpvnb180ApH1uGBCt7uObA&usqp=CAU',
        }
  ],    
  loading: false,
};
  ...
  reducers: {
    ...
    removeAnimal: (state, action: PayloadAction<number>) => {
      state.animals = state.animals.filter(
        (animals) => animals.id !== action.payload
      );
      window.localStorage.setItem(`animals`, JSON.stringify(state.animals));
    },
  },
...
// setAllAnimals
export const { addAnimal, removeAnimal, setAllAnimals, setLoading } =
  animalSlice.actions;
